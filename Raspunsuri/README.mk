...
Este important ca `OnResize()` să fie apelată deoarece, inițial, fereastra poate avea dimensiunea 0x0, iar această metodă asigură că OpenGL este configurat corect pentru dimensiunea dorită a ferestrei.

## Ce semnifică parametrii metodei `CreatePerspectiveFieldOfView()` și care sunt domeniile lor de valori?
Metoda `CreatePerspectiveFieldOfView()` definește câmpul vizual în care obiectele sunt afișate pe ecran. Parametrii principali includ:

- **fovY**: unghiul de vedere vertical, de exemplu `MathHelper.DegreesToRadians(60)` pentru un câmp vizual de 60 de grade.
